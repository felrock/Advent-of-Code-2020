...
using XYZ = std::tuple<int, int, int, int>;

std::string xyzToString(const XYZ& xyz)
{
    return std::to_string(std::get<0>(xyz)) + "," + std::to_string(std::get<1>(xyz)) +
           "," + std::to_string(std::get<2>(xyz)) + "," + std::to_string(std::get<3>(xyz));
}
...
XYZ stringToXyz(const std::string& str)
{
    ...
    return XYZ(nums[0], nums[1], nums[2], nums[3]);
}
...
int countNeighbors(const XYZ& xyz, const std::unordered_map<std::string, XYZ>& world, const std::vector<XYZ>& neighbors)
{
    ...
    int w1 = std::get<3>(xyz);

    for(const auto& n : neighbors)
    {
        XYZ txyz = std::make_tuple(x1+std::get<0>(n), y1+std::get<1>(n), z1+std::get<2>(n), w1+std::get<3>(n));
...
int main(int argc, char** argv)
{
    ...
    while(std::getline(file, s))
    {
        for(const auto& chr : s)
        {
            if(chr == '#')
            {
                XYZ xyz = std::make_tuple(x, y, z, w);
    ...
    for(const auto& v1 : val)
    {
        for(const auto& v2 : val)
        {
            ...
            {
                for(const auto& v4 : val)
                {
                    if(v1 != 0 || v2 != 0 || v3 != 0 || v4 != 0)
                    {
                        neighbors.push_back(std::make_tuple(v1,v2,v3, v4));
                    }
                }
            }
        }
    ...
    while(iteration < max_iterations)
    {
        ...
        for(const auto& xyz : world)
        {
            ...
            int w1 = std::get<3>(xyz.second);

            for(const auto& v : neighbors)
            {
                XYZ neighbor_pos = std::make_tuple(x1+std::get<0>(v), y1+std::get<1>(v), z1+std::get<2>(v), w1+std::get<3>(v));
